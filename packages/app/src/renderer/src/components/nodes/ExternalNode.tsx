...
function ExternalImpl({ data, width, selected }: NodeProps): React.JSX.Element {
  ...
  return (
    <motion.div
      ...
      style={{ width }}
      className={[
        'rounded-lg border-2 border-dashed bg-ink-900/60 px-3 py-2',
    ...
    >
      ...
      <button
        ...
        className={[
          ...
          clickable ? 'hover:bg-ink-800/50 cursor-pointer rounded' : 'cursor-default',
        ].join(' ')}
      ...
      >
        ...
        {wc.description ? (
          <div className="text-ink-600 text-[10px] text-center max-w-full truncate">{wc.description}</div>
        ) : null}
      </button>
    </motion.div>
  )
